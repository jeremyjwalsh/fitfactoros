...
Then add this Sheet check word for word: *"Before we start, make sure your FitFactorOS Sheet is open in Google Sheets. If you downloaded the file, upload it to Google Drive, open it, and choose **File > Save as Google Sheets**. You'll paste your answers there at the end."*
...
1. Show a short preview table with two columns, **Field** and **Your answer**, covering all 18 fields in the order in `references/profile-map.md`.
2. Say: "Here's your answer block. Tap **Copy**, open your FitFactorOS Sheet in Google Sheets, go to the **Profile** tab, click cell **B2**, and paste. Each answer lands on its own row. (Still have the downloaded file? Upload it to Google Drive, open it, and choose **File > Save as Google Sheets** first.)"
3. Give the block in a single code block: **exactly 18 lines**, answers only, no field names, following every rule in `references/profile-map.md`. The first line is the Gmail address; the last line is today's date as YYYY-MM-DD.
4. Close in 2–3 sentences: their Profile is set. If the Gmail address was skipped, tell them the inbox links won't work until they type it into cell B2. Once Research is available, the next step is to share a job or company they're considering and say "fit factor this."
